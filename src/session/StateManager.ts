...
/**
 * SessionStateManager: 负责管理 Opengravity 的运行状态、权限和行为约束
 * 独立于 UI，实现状态的持久化与协议生成
 */
export class SessionStateManager {
    private _mode: OpengravityMode = OpengravityMode.Manual;
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            this._mode = data.mode || OpengravityMode.Manual;
        } catch (e) {
            this._mode = OpengravityMode.Manual;
        }
    }

    get mode(): OpengravityMode {
        return this._mode;
    }
     ...
     * 切换模式并持久化
     */
    async setMode(newMode: OpengravityMode): Promise<string> {
        this._mode = newMode;
        await this.save();
     ...
     */
    canSpeak(): boolean {
        return this._mode !== OpengravityMode.Silent;
    }
    ...
    isAutonomous(): boolean {
        return this._mode === OpengravityMode.Automatic;
    }
